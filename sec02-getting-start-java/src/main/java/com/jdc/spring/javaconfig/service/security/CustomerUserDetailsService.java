package com.jdc.spring.javaconfig.service.security;
...
import com.jdc.spring.javaconfig.service.CustomerService;
...
public class CustomerUserDetailsService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		return service.findByEmail(username)
				.map(customer -> User.withUsername(username).authorities("Customer").password(customer.getPassword())
						.accountLocked(customer.isLocked()).disabled(!customer.isActivated())
						.accountExpired(isExpired(customer)).credentialsExpired(isCredentialExpired(customer)).build())
