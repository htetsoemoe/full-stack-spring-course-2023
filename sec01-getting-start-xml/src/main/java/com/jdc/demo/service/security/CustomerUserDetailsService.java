...
public class CustomerUserDetailsService implements UserDetailsService{
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		return result.map(customer -> User.withUsername(username)
				...
				.disabled(!customer.isActivated()) // if user is not activated
				.accountExpired(isExpried(customer)) // if user account is lock
				.credentialsExpired(isCredentialExpired(customer)) //if user password is expired
				.build())
	...
	private boolean isExpried(Customer customer) {
